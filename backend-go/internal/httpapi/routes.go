...
func APIRoutes() []Route {
	return []Route{
		...
		route("GET", "/api/auth/status", AuthNone, handleNotImplemented),
		route("GET", "/api/auth/me", AuthAny, handleNotImplemented),
		route("GET", "/api/auth/settings", AuthOwner, handleNotImplemented),
		route("PUT", "/api/auth/settings", AuthOwner, handleNotImplemented).withJSONBody(),
		route("PUT", "/api/auth/password", AuthAny, handleNotImplemented).withJSONBody(),
		route("POST", "/api/auth/register", AuthNone, handleNotImplemented).withJSONBody(),
		route("POST", "/api/auth/login", AuthNone, handleNotImplemented).withJSONBody(),
		route("POST", "/api/auth/invitations", AuthOwner, handleNotImplemented).withJSONBody(),
		route("GET", "/api/users", AuthOwner, handleNotImplemented),
		route("POST", "/api/users", AuthOwner, handleNotImplemented).withJSONBody(),
		route("PUT", "/api/users/{id}", AuthOwner, handleNotImplemented).withPattern(`/api/users/(user_[A-Za-z0-9]+)`).withSample("/api/users/user_abc123").withJSONBody(),
		route("DELETE", "/api/users/{id}", AuthOwner, handleNotImplemented).withPattern(`/api/users/(user_[A-Za-z0-9]+)`).withSample("/api/users/user_abc123"),
		route("GET", "/api/cli-auth/request", AuthNone, handleNotImplemented).withRateLimit("cli_auth_request_status"),
		route("POST", "/api/cli-auth/request", AuthNone, handleNotImplemented).withRateLimit("cli_auth_request_create"),
		route("POST", "/api/cli-auth/approve", AuthBrowserSession, handleNotImplemented).withJSONBody().withRateLimit("cli_auth_approve"),
