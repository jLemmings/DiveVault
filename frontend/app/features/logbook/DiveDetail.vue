  ...
  computed: {
      ...
      const metrics = [
        {
          label: "Duration",
          icon: "timer",
          value: durationParts(this.dive?.duration_seconds).value,
          unit: durationParts(this.dive?.duration_seconds).unit,
          tone: "text-primary"
        },
        {
          label: "Max Depth",
          icon: "south",
          value: depthParts(this.dive?.max_depth_m).value,
          unit: depthParts(this.dive?.max_depth_m).unit,
          tone: "text-primary"
        },
        {
          label: "Avg Temp",
          icon: "thermostat",
          value: temperatureParts(surfaceTemperature(this.dive)).value,
          unit: temperatureParts(surfaceTemperature(this.dive)).unit,
          tone: "text-tertiary"
        },
    ...
    diveSystemCards() {
      ...
      return [
        ...
        {
          label: "SAC Rate",
          value: typeof sac === "number" ? `${sac.toFixed(1)} L/min` : "No SAC",
          detail: "Surface air consumption",
          icon: "speed"
        }
      ];
    },
...
<template>
  <section v-if="dive" class="relative overflow-hidden bg-surface-container-low p-6 shadow-panel md:p-8">
    ...
    <div
      class="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(156,240,255,0.35),transparent_22rem)] pointer-events-none"
    ></div>
    <div class="relative z-10">
      <section class="space-y-8 md:hidden">
        <header class="space-y-4">
          <div class="flex items-center justify-between gap-3">
            <UButton
              @click="closeDetail"
              class="inline-flex items-center gap-2 rounded-lg bg-surface-container-high px-4 py-2 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary"
            >
            ...
            <div class="flex items-center gap-2">
              <UButton
                ...
                @click="removeDive()"
                :disabled="isDeleting"
                aria-label="Remove dive"
                title="Remove dive"
                class="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-error-container/20 text-on-error-container transition-colors hover:bg-error-container/30 disabled:opacity-50"
              >
                <span class="material-symbols-outlined text-sm">delete</span>
              </UButton>
              <span
                class="rounded bg-surface-container-high px-3 py-2 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-secondary"
                >Dive {{ displayDiveIndex }}</span
              >
            </div>
          </div>
          ...
          <section class="relative min-h-[17rem] overflow-hidden rounded-[1.5rem] p-6">
            <div
              class="absolute inset-0 bg-[radial-gradient(circle_at_20%_20%,rgba(156,202,255,0.22),transparent_22%),radial-gradient(circle_at_80%_25%,rgba(255,183,125,0.18),transparent_18%),linear-gradient(180deg,#0f2a3f_0%,#021523_100%)]"
            ></div>
            ...
            <div class="relative z-10 flex h-full flex-col justify-end space-y-2">
              <div class="flex flex-wrap items-center gap-2">
                <span
                  class="rounded bg-tertiary-container/40 px-2 py-1 font-label text-[10px] font-bold uppercase tracking-[0.16em] text-tertiary"
                  >{{ diveModeLabel(dive) }}</span
                >
                <span class="font-label text-[10px] font-bold uppercase tracking-[0.16em] text-on-surface-variant">{{
        ...
        <section class="overflow-hidden rounded-xl shadow-panel">
          <div class="grid grid-cols-3 gap-px bg-background/20">
            ...
            <div class="bg-surface-container-high p-4 text-center">
              ...
              <div class="mt-1 flex items-baseline justify-center gap-1">
                <span class="font-headline text-xl font-bold text-tertiary">{{
                  formatTemperature(surfaceTemperature(dive)).replace(" C", "")
                }}</span>
        ...
        <section v-if="hasDepthProfile" class="space-y-4">
          ...
          <div class="relative overflow-hidden rounded-xl bg-surface-container-low p-4">
            ...
            <div class="relative grid grid-cols-[auto_1fr_auto] gap-3">
              ...
              <div class="relative h-48">
                <svg class="h-48 w-full" viewBox="0 0 800 250" preserveAspectRatio="none">
                  ...
                  ></circle>
                  <circle
                    v-if="hasPressureProfile && profileHover && profileHover.pressureY !== null"
                    :cx="profileHover.x"
                    :cy="profileHover.pressureY"
                    r="4.5"
                    fill="#FFB77D"
                    stroke="#0b2940"
                    stroke-width="2"
                ...
                >
                  ...
                  <p v-if="profileHover.pressureLabel" class="mt-1 text-xs font-semibold text-tertiary">
                    Air: {{ profileHover.pressureLabel }}
                  </p>
                </div>
              </div>
              <div v-if="hasPressureProfile" class="relative h-48 w-12 pl-1">
                <span
            ...
            <div
              class="relative mt-3 flex items-center justify-between font-label text-[8px] font-bold uppercase tracking-[0.16em] text-on-surface-variant/50"
            >
        ...
        <section v-if="hasLogbookMetadata" class="space-y-4">
          ...
          <div class="space-y-3">
            <article
              v-for="item in logbookMetadata"
              :key="'mobile-logbook-' + item.key"
              class="rounded-xl bg-surface-container-high/70 p-4"
            >
      ...
      <section class="hidden space-y-8 md:block">
        <header
          class="relative min-h-[19rem] overflow-hidden rounded-[2rem] border border-primary/10 bg-surface-container-low shadow-panel"
        >
          <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_55%_12%,rgba(42,160,178,0.38),transparent_18rem),linear-gradient(120deg,rgba(0,21,37,0.88),rgba(0,21,37,0.48)_45%,rgba(0,21,37,0.9)),linear-gradient(165deg,transparent_0_28%,rgba(156,202,255,0.16)_29%,transparent_31%_42%,rgba(156,202,255,0.1)_43%,transparent_45%)]"
          ></div>
          ...
          <div class="relative z-10 flex items-start justify-between gap-5 p-8">
            <UButton
              @click="closeDetail"
              class="inline-flex items-center gap-2 rounded-xl border border-primary/20 bg-background/35 px-4 py-3 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary shadow-panel transition-colors hover:bg-primary/10"
            >
              <span class="material-symbols-outlined text-base">arrow_back</span>
              Back To Logs
            </UButton>
            <div v-if="!publicView" class="flex gap-2">
              <UButton
                @click="openDiveEditor(dive.id)"
                aria-label="Edit dive"
                title="Edit dive"
                class="group inline-flex h-11 w-11 items-center justify-center rounded-xl border border-primary/20 bg-background/35 text-secondary transition-colors hover:bg-primary/10 hover:text-primary"
              >
                <span
                  class="material-symbols-outlined text-[21px] leading-none transition-transform group-hover:-rotate-6 group-hover:scale-110"
                  >edit</span
                >
              </UButton>
              <UButton
                ...
                aria-label="Remove dive"
                title="Remove dive"
                class="inline-flex h-11 w-11 items-center justify-center rounded-xl border border-error/20 bg-error-container/15 text-on-error-container transition-colors hover:bg-error-container/30 disabled:opacity-50"
              >
          ...
          <div class="absolute bottom-8 left-8 right-8 z-10">
            <div
              class="flex flex-wrap items-center gap-3 font-label text-[11px] font-bold uppercase tracking-[0.18em] text-on-surface-variant"
            >
            ...
            <h3 class="mt-3 max-w-5xl font-headline text-5xl font-bold leading-[0.95] tracking-tight text-on-surface xl:text-6xl">
              {{ diveSiteTitle(dive) }}
            </h3>
            <div class="mt-5 flex flex-wrap gap-10">
              <div v-for="metric in heroMetricTiles" :key="'hero-metric-' + metric.label" class="min-w-[7rem]">
                ...
                <p class="mt-1 font-headline text-2xl font-bold text-on-surface">
                  {{ metric.value }}<span class="ml-1 text-sm text-on-surface-variant">{{ metric.unit }}</span>
                </p>
              </div>
            </div>
          </div>
          ...
          <main class="col-span-12 space-y-6 xl:col-span-8">
            ...
            >
              <div class="mb-6 flex items-center justify-between gap-4">
                ...
                <span
                  class="rounded-full bg-surface-container-high px-3 py-1 text-[10px] font-black uppercase tracking-[0.2em] text-primary"
                  >{{ logbookMetadata.length }} fields</span
                >
              </div>
              <div class="grid grid-cols-3 gap-4">
                <article
                  v-for="item in logbookMetadata"
                  :key="'desktop-logbook-' + item.key"
                  class="rounded-xl bg-surface-container-high/70 p-4"
                >
            ...
            <section class="overflow-hidden rounded-[1.5rem] border border-primary/10 bg-surface-container-low/80 shadow-panel">
              <div v-if="mapPreview" class="relative min-h-[20rem]">
                ...
                <div
                  class="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,21,37,0.06)_0%,rgba(0,21,37,0.08)_38%,rgb(var(--color-background)_/_0.82)_100%),radial-gradient(circle_at_center,rgb(var(--color-primary)_/_0.16),transparent_17rem)]"
                ></div>
                <div
                  class="absolute -translate-x-1/2 -translate-y-1/2"
                  :style="{ left: mapPreview.imageMarkerLeft, top: mapPreview.imageMarkerTop }"
                >
                  <span class="relative flex h-16 w-16 items-center justify-center">
                    ...
                    <span
                      class="relative flex h-5 w-5 items-center justify-center rounded-full bg-tertiary shadow-[0_0_22px_rgb(var(--color-tertiary)_/_0.7)]"
                    >
              ...
              <div v-else class="relative flex min-h-[20rem] flex-col justify-end overflow-hidden p-6">
                <div
                  class="absolute inset-0 bg-[linear-gradient(135deg,rgb(var(--color-surface-container-high)_/_0.7),rgb(var(--color-surface-container-low)_/_0.9)),radial-gradient(circle_at_70%_25%,rgb(var(--color-primary)_/_0.18),transparent_18rem)]"
                ></div>
          ...
          <section class="col-span-12 pt-8">
            ...
            <div class="grid grid-cols-[repeat(auto-fit,minmax(12rem,1fr))] gap-4">
              <article
                ...
                class="flex items-center gap-4 rounded-xl border border-primary/10 bg-surface-container-low/75 p-4"
              >
                <span
                  class="material-symbols-outlined flex h-11 w-11 items-center justify-center rounded-lg bg-surface-container-high text-primary"
                  >{{ item.icon }}</span
                >
                <div class="min-w-0">
