  ...
  <section class="space-y-10 text-on-surface">
    <section class="space-y-6 md:hidden">
      <div class="flex items-end justify-between">
        ...
        <span
          ...
          >{{ pendingDives.length }} Imported</span
        >
      </div>
        ...
        >
          ...
          <div class="space-y-5 p-5">
            <div class="flex items-start justify-between gap-4">
              <div>
                <p class="font-label text-xs font-bold uppercase tracking-[0.12em] text-on-surface-variant opacity-70">
                  IMPORT_ID: {{ paddedDiveIndex(dive) }}
                </p>
                <div class="mt-2 space-y-1.5">
                  <h3 class="font-headline text-[1.7rem] font-bold tracking-tight text-primary tabular-nums">
            ...
            <div class="grid grid-cols-2 gap-3">
              <div class="rounded-lg border-l-2 border-primary bg-surface-container-high p-3">
                ...
                <p class="font-headline text-2xl font-bold text-primary">
                  {{ formatDepthNumber(dive.max_depth_m) }}<span class="ml-1 text-xs font-normal opacity-60">M</span>
                </p>
              </div>
              <div class="rounded-lg border-l-2 border-secondary bg-surface-container-high p-3">
                ...
                <p class="font-headline text-2xl font-bold text-secondary">
                  {{ durationMinutes(dive) }}<span class="ml-1 text-xs font-normal opacity-60">MIN</span>
                </p>
              </div>
            ...
            <div class="flex flex-wrap gap-2">
              ...
              <span
                v-if="missingFields(dive).length === 0"
                class="inline-flex items-center gap-1 rounded bg-primary/10 px-2 py-1 font-label text-[10px] font-bold uppercase tracking-[0.14em] text-primary"
              >
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <UButton
                ...
                :disabled="isDeleting(dive.id)"
                class="rounded-lg bg-error-container/20 px-4 py-3 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-on-error-container disabled:opacity-50"
              >
    ...
    <section class="hidden space-y-10 md:block">
      <header class="space-y-6">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div class="flex gap-3">
            <UButton
              @click="setView('logs')"
              class="inline-flex items-center gap-2 bg-surface-container-high px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-surface transition-colors hover:text-primary"
            >
            ...
            <UButton
              @click="fetchDives"
              class="inline-flex items-center gap-2 bg-surface-container-high px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-primary transition-colors hover:bg-surface-container-highest"
            >
          ...
          <div class="relative grid gap-6 xl:grid-cols-[minmax(0,1.65fr)_minmax(260px,0.82fr)] xl:items-end">
            <div class="space-y-5">
              <p class="max-w-3xl text-sm leading-7 text-on-surface-variant">
                Imported dives from the DiveVault importer stay in the staging queue until the diver completes the required registry
                metadata. Open a record to add the dive site before it enters the permanent logbook. Buddy, guide, and notes can be added
                when useful.
              </p>
              <div class="flex flex-wrap items-center gap-3">
                <span
                  class="inline-flex items-center gap-2 bg-background/40 px-3 py-2 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary"
                >
                  <span class="material-symbols-outlined text-sm">workspace_premium</span>
                  Metadata Workflow Active
                </span>
                <span
                  v-if="nextPendingDive"
                  class="inline-flex items-center gap-2 bg-tertiary/10 px-3 py-2 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-tertiary"
                >
        ...
        <div v-if="importError" class="border border-error/20 bg-error-container/20 px-5 py-4 text-sm text-on-error-container shadow-panel">
          {{ importError }}
        </div>
      </header>
      ...
      <section v-if="pendingDives.length" class="space-y-4">
        ...
        >
          <div
            class="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/60 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-60"
          ></div>
          <div class="grid gap-0 2xl:grid-cols-[212px_minmax(0,1fr)_260px]">
            <div class="border-b border-primary/10 bg-surface-container-highest/20 p-6 2xl:border-b-0 2xl:border-r">
              <div class="flex items-start justify-between gap-3 2xl:block">
                <div>
                  ...
                  <p class="mt-2 whitespace-nowrap font-headline text-[1.7rem] font-bold tracking-tight text-primary tabular-nums">
                    {{ compactDateStamp(dive.started_at) }}
                  </p>
            ...
            <div class="min-w-0 space-y-6 p-6 lg:p-8">
              <div class="flex flex-wrap items-start justify-between gap-4">
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    ...
                    >
                    <span
                      v-if="isNightDive(dive)"
                      class="bg-primary/10 px-3 py-1 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary"
                      >Night Dive</span
                    >
                    <span class="bg-background/35 px-3 py-1 font-label text-[10px] font-bold uppercase tracking-[0.18em] text-secondary"
                      >{{ missingFields(dive).length }} Missing</span
                    >
                  </div>
                  <h4 class="mt-4 font-headline text-2xl font-bold tracking-tight text-on-surface">{{ dive.vendor }} {{ dive.product }}</h4>
                  <p class="mt-2 text-sm text-on-surface-variant">
                    {{ formatDate(dive.started_at) }} telemetry import awaiting registry metadata.
                  </p>
                </div>
              </div>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="border border-primary/10 bg-surface-container-high/35 p-4">
                  ...
                  <p class="mt-3 font-headline text-3xl font-bold leading-none text-primary">
                    {{ formatDepthNumber(dive.max_depth_m) }}<span class="ml-1 text-xs font-normal text-secondary">M</span>
                  </p>
                </div>
                <div class="border border-primary/10 bg-surface-container-high/35 p-4">
              ...
              <div class="space-y-3">
                <div class="flex items-center justify-between gap-4">
                  ...
                  <p class="font-label text-[10px] font-bold uppercase tracking-[0.18em] text-primary">
                    {{ completionForDive(dive) }}% Ready
                  </p>
                </div>
                <div class="h-1 overflow-hidden bg-surface-container-highest">
                  <div
                    class="h-full bg-gradient-to-r from-primary to-tertiary transition-all duration-300"
                    :style="{ width: completionForDive(dive) + '%' }"
                  ></div>
                </div>
                <div class="flex flex-wrap gap-2">
                  <span
            ...
            <div
              class="flex flex-col justify-between gap-5 border-t border-primary/10 bg-surface-container-high/18 p-6 lg:p-8 2xl:border-l 2xl:border-t-0"
            >
              <div class="space-y-4">
                ...
                <p class="text-sm leading-6 text-on-surface-variant">
                  Open this imported dive to complete the missing metadata and commit it into the main logbook.
                </p>
              </div>
              ...
              <div class="space-y-4">
                <div class="rounded border border-primary/10 bg-background/35 p-4">
                  ...
                  <p class="mt-2 text-sm font-semibold" :class="missingFields(dive).length ? 'text-tertiary' : 'text-primary'">
                    {{ missingFields(dive).length ? missingFields(dive)[0].label : "Ready To Commit" }}
                  </p>
                </div>
                <div class="grid grid-cols-2 gap-3">
                  <UButton
                    @click="selectImportDive(dive.id)"
                    class="w-full bg-primary px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-primary transition-all hover:brightness-110"
                  >
                    Edit Imported Dive
                  </UButton>
                  <UButton
                    @click="removeDive(dive.id)"
                    :disabled="isDeleting(dive.id)"
                    class="w-full bg-error-container/20 px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-error-container transition-colors hover:bg-error-container/30 disabled:opacity-50"
                  >
                    {{ isDeleting(dive.id) ? "Removing..." : "Remove Dive" }}
                  </UButton>
                </div>
      ...
      <section v-else class="space-y-6 bg-surface-container-low p-10 shadow-panel">
        ...
        <p class="max-w-2xl text-sm leading-7 text-on-surface-variant">
          The imported queue is empty. Continue with the dive log or inspect the most recent telemetry detail.
        </p>
        <div class="flex flex-wrap gap-3">
          <UButton
            @click="setView('logs')"
            class="bg-primary px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-on-primary"
            >Return To Logs</UButton
          >
          <UButton
            @click="fetchDives"
            class="bg-surface-container-high px-5 py-3 font-label text-[10px] font-bold uppercase tracking-[0.2em] text-primary"
            >Refresh Queue</UButton
          >
        </div>
      </section>
