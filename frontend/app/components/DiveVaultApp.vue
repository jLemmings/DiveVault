...
function singleRouteParam(value) {
...
const DashboardView = defineAsyncComponent(() => import("~/components/views/Dashboard.vue"));
const DiveDetailView = defineAsyncComponent(() => import("~/components/views/DiveDetail.vue"));
const DiveImportEditorView = defineAsyncComponent(() => import("~/components/views/ImportEdit.vue"));
const DiveImportView = defineAsyncComponent(() => import("~/components/views/Imports.vue"));
const EquipmentView = defineAsyncComponent(() => import("~/components/views/Equipment.vue"));
const LogbookEditorView = defineAsyncComponent(() => import("~/components/views/LogbookEdit.vue"));
const LoginView = defineAsyncComponent(() => import("~/components/views/Login.vue"));
const LogsView = defineAsyncComponent(() => import("~/components/views/Logs.vue"));
const ManualDiveEntryView = defineAsyncComponent(() => import("~/components/views/ManualDive.vue"));
const PublicProfileView = defineAsyncComponent(() => import("~/components/views/PublicProfile.vue"));
const SettingsView = defineAsyncComponent(() => import("~/components/views/Settings.vue"));
...
export default {
  ...
  methods: {
    ...
    currentRouteState() {
      return routeToState({ path: this.routePathOverride || this.nuxtRoute.path });
    },
    syncRouteMode() {
      this.publicRouteSlug = this.currentRouteState().publicRouteSlug || "";
    },
    handleRouteNavigation() {
      if (this.routePathOverride && this.nuxtRoute.path === this.routePathOverride) {
        this.routePathOverride = null;
      }
      this.syncRouteMode();
    ...
    async navigateToAppRoute(path, options = {}) {
      if (!path || this.nuxtRoute.path === path) {
        return;
      }
      if (this.routePathOverride && this.routePathOverride !== path) {
        this.routePathOverride = null;
      }
      const method = options.replace ? "replace" : "push";
      await this.nuxtRouter[method](path);
    },
  ...
  async mounted() {
    ...
    if (legacyPath) {
      this.routePathOverride = legacyPath;
      await this.nuxtRouter.replace(legacyPath);
    }
