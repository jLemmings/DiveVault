...
def require_auth(handler) -> dict | None:
    ...
    try:
        return verifier.verify_request(handler.headers)
...
def read_json_body(handler, *, max_bytes: int | None = None) -> dict | None:
    max_json_body_bytes = int(max_bytes if max_bytes is not None else getattr(handler.server, "max_json_body_bytes", 1024 * 1024))
